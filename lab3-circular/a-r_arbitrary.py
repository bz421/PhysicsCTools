import numpy as np
from mpmath import mp, mpf, log, linspace
...
xerr = [0.0000005 for _ in range(len(x))]
yerr = [0.0000005 for _ in range(len(y))]
...
# Generate fit lines
log_xfit = [mpf(val) for val in linspace(min(log_x) - mpf(0.5), max(log_x) + mpf(0.5), 100)]
